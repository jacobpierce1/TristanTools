...
matplotlib.use('Agg') # <-- THIS MAKES IT FAST!
...
if len( sys.argv ) < 2 :
    print( 'ERROR: must supply timestep' )
    sys.exit( 1 ) 
...
def make_jet_movie_new( analyzer, timestep,
                        # figsize, title, xlabel, ylabel, 
    ...
    jet_movie_data_getter( timestep, analyzer )
    xslice_index = int( analyzer.data.dens[ timestep ].shape[0] / 2 )
    zslice_index = int( analyzer.data.dens[ timestep ].shape[2] / 2 )
    ...
    fig.suptitle( 'Time = %.2e' % analyzer.data.time[ timestep ][0] )
    
    for i in range(2) :
        for j in range(3) :

            data = analyzer.data[ dens_data_names[j] ][ timestep ][ dens_slices[i] ] 
            ...
            data = [ analyzer.data[ key ][ timestep ][ dens_slices[i] ]
...
ani = make_jet_movie_new( analyzer, timestep,
                          # ( 9, 6 ), '', '$x$ (cells)', '$y$ (cells)',
                          # slice_index, 0, 
                          cmap, interpolation,
